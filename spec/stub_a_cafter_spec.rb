  ...
  context "クラスを操作対象として指定した場合" do
    context "存在するクラスメソッドを指定した場合" do
      ...
      it "after フックが作成されていること" do
        expect(target_class.method_defined? after_zweit_method_name).to be true
      end

      it "指定していない before フックは作成されていないこと" do
        expect(target_class.method_defined? before_zweit_method_name).to be false
      end
      ...
      context "ブロックを伴わないメソッドの場合" do
        ...
        it "メソッド実行ののちに after フックも実行されること" do
          value = target.zweit(log, 1, 2)
          expect(value).to eq [:origin, [1, 2], "after (zweit)", [1, 2]]
        end
      end
      ...
      context "ブロックを伴うメソッドの場合" do
        ...
        it "メソッド実行の後に after フックも実行されること" do
          ...
          expect(value).to eq [:origin, :origin_block, [9, 8],
                               "after (dritt)", [9, 8]]
        end
      end
    end
    ...
    context "存在しないメソッドを指定した場合" do
      ...
      it "エラーが発生すること" do
        expect{
          StubA.new(target).cafter(:foo) {|_| :a }
        }.to raise_error
      end
